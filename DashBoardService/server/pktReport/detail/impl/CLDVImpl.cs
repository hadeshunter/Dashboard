    ...
    public class CLDVImpl : ICLDV
    {
        ...
        private dynamic getCLDV_khl_oracle(string startime, string endtime)
        {
            ...
            if (checkDate == true)
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SqlCommand(@"select tuan, CONVERT(DATETIME,ngay,103) ngay, donvi_cha_id, sl from [dbo].TK_KhongHaiLong_CLDV where CONVERT(DATETIME,ngay,103) between CONVERT(DATETIME,'" + startime + "',103) and CONVERT(DATETIME,'" + endtime + "',103)", conn))
                    {
