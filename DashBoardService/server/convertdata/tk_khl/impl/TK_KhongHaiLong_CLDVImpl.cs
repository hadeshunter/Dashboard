...
{
    public class TK_KhongHaiLong_CLDVImpl: ITK_KhongHaiLong_CLDV
    {
        private IConfiguration m_configuration;
        public TK_KhongHaiLong_CLDVImpl(IConfiguration _configuration)
        {
            m_configuration = _configuration;
        }

        public void insertKhongHaiLong_CLDV(List<TK_KhongHaiLong_CLDV> listData)
        {
            ...
            using (SqlConnection con = new SqlConnection(conn))
            {
                ...
                try
                {
                    ...
                    {
                        var item = listData[i];
                        var query = "INSERT INTO TK_KhongHaiLong_CLDV(tuan,ngay,donvi_cha_id,sl,timeinsert)" +
        ...
        public dynamic toDataConvert(string startime, string endtime)
        {
            try
            {
                return checkconvertKhongHaiLong_CLDV(startime, endtime);
            }
            catch (Exception e)
            {
                return e;
        ...
        public void convertKhongHaiLong_CLDV(string startime, string endtime)//string startime, string endtime
        {
            try
            {

                List<TK_KhongHaiLong_CLDV> result = new List<TK_KhongHaiLong_CLDV>();
                ...
                {
                    var query = "dashboard.TK_KhongHaiLong_CLDV_date";
                    result = SqlMapper.Query<TK_KhongHaiLong_CLDV>(conn, query, param: dyParam, commandType: CommandType.StoredProcedure).AsList<TK_KhongHaiLong_CLDV>();
                    conn.Close();
                }
                insertKhongHaiLong_CLDV(result);
            }
            catch (Exception e)
            {
            }
        }
        private dynamic checkconvertKhongHaiLong_CLDV(string startime, string endtime)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand(@"select endtimeupdate,endtime from TimeConvert where timeid = 4", conn))
                {
                    ...
                    {
                        convertKhongHaiLong_CLDV(startime, endtime);
                    }
                    ...
                    if (time2 > time1)
                    {
                        ...
                        using (var cmd1 = new SqlCommand(@"update TimeConvert set endtime = (convert(datetime,'" + time2.AddDays(1).ToString("dd/MM/yyyy") + "', 103)),timeinsert = " + timeinsert + " where timeid = 4", conn))
                        {
                            cmd1.ExecuteNonQuery();
                        }
                        convertKhongHaiLong_CLDV(etime.ToString("dd/MM/yyyy"), endtime); //convert tu ngay cuoi den ngay lay du lieu
                        return true;
                    }
                    conn.Close();
